...
Heatmaps are another way of displaying muti-dimensional data in a single figure. We use the synthesized data from `ethnobotanyR` for this heatmap example [@ethnobotanyR].
...
Here demonstrated various graphical options to visualize uncertainty intervals (e.g.outcomes of Monte Carlo simulation)
...
We can use the function `mcmc_intervals()`or `mcmc_areas()`to plot the data set
...
We can also use `geom_density()`in `ggplot2`to compare the spread of different distributions:
...
Use `geom_histogram()`to show the histogram of the data in comparison:
...
Here demonstrated an option to visualize a set of variable with multiple attributes.

Create example data frame: Assume that we have five independent variables that are involved in the model to predict yield (above example). After running a regression analysis and performing value of information analysis, we got a data set with three attribute for each variable:
...
After having all element plot, use cowplot to put everything together:
