...
Rows as names and columns as variables `factor_variable`
...
To show adial box plot with a data set and grid with four factor variables and one continuous.
...
To demonstrate the spider plot data visualization we create a function to obtain straight lines using `match.arg` from base R.
...
In order to neatly close the plot we add an empty level to the daat set (a quasi-blank variable) which needs the same value as level 1. For this to work both factors ("A" and "B" in our case) need this correction.
...
We add the factor, variable, and value togteher with the blank variable to a data set using `data.frame`. 

```{r}
data <- data.frame(factor = factor, variable = variable, value = value)
```

Plto with the `ggplot2` library.

```{r}
ggplot(data, aes(as.numeric(data$variable),value)) + 
  coord_radar() +
